...
object KeyDeriver {
    ...
    fun keyFor(project: Project, file: VirtualFile): Key? {
        val vcsRoot = ProjectLevelVcsManager.getInstance(project).getVcsRootFor(file)
    ...
    fun keyFor(project: Project, filePath: FilePath): Key? {
        val vf = filePath.virtualFile
        if (vf != null) return keyFor(project, vf)

        val vcsRoot = ProjectLevelVcsManager.getInstance(project)
    ...
    private fun keyForNoVcs(file: VirtualFile): Key =
        Key(repoRoot = Key.NO_VCS, branch = Key.NO_BRANCH, path = file.path)
}
